...
function AddCollegeForm({ onSubmit, onCancel, submitting, formError }) {
  const [form, setForm] = useState(initialState);
...
function CollegeTable({ colleges, internCounts }) {
  return (
    <div className="mx-auto mt-6 w-full max-w-6xl overflow-hidden rounded-xl bg-white shadow-sm">
      {/* Mobile / small screens: stacked cards */}
      <div className="divide-y divide-slate-50 sm:hidden">
      ...
      <div className="hidden overflow-x-auto sm:block">
        <table className="w-full min-w-[720px] border-collapse text-left">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="px-4 py-4 text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">College</th>
              <th className="px-4 py-4 text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">Code</th>
              <th className="px-4 py-4 text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">Location</th>
              <th className="px-4 py-4 text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">Contact Person</th>
              <th className="px-4 py-4 text-right text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">Interns</th>
              <th className="px-4 py-4 text-xs font-medium uppercase tracking-wider text-slate-400 lg:px-6">Status</th>
            </tr>
          </thead>
          <tbody>
            {colleges.map((college, idx) => (
              <tr
                key={college.id}
                className={`${idx % 2 === 1 ? "bg-slate-50/60" : "bg-white"} border-b border-slate-50 last:border-0`}
              >
                <td className="px-4 py-4 text-sm font-semibold text-slate-800 lg:px-6">{college.name}</td>
                <td className="px-4 py-4 font-mono text-sm text-slate-400 lg:px-6">{college.code}</td>
                <td className="px-4 py-4 text-sm text-slate-500 lg:px-6">{college.address || "—"}</td>
                <td className="px-4 py-4 text-sm text-slate-500 lg:px-6">{college.contactPerson || "—"}</td>
                <td className="px-4 py-4 text-right text-sm font-semibold text-slate-800 lg:px-6">
                  {internCounts[college.id] || 0}
                </td>
                <td className="px-4 py-4 lg:px-6">
...
function Colleges() {
  ...
  const handleRegister = async (formData) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || err.message || "Failed to add college");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return <div className="p-6 text-slate-500">Loading colleges...</div>;
  }
  ...
  return (
    <div className="sm:p-6 min-h-screen">
      ...
      <CollegeTable colleges={colleges} internCounts={internCounts} />
